...
For machines that don't necessarily need the language servers the module exposes an option to disable all such components.
...
******* DONE Add path completion :feat:
CLOSED: [2023-06-07 Wed 11:58]
...
******* TODO Check out [[https://justingarrison.com/blog/2023-06-05-zsh-global-aliases/][global aliases]] :to_think:
******* TODO Consider adding [[https://github.com/samwho/spacer][spacer]] :to_think:
...
******* TODO Check out [[https://www.hschne.at/git-aliases/][git aliases]] :to_think:
...
***** Tmux
:PROPERTIES:
:header-args:nix+: :tangle nixosModules/tmux/default.nix
:END:

Imported [[(tmux-module-import)][here]].
...
****** Tmux.conf
:PROPERTIES:
:header-args:conf+: :tangle nixosModules/tmux/tmux.conf
:END:
...
:PROPERTIES:
:header-args:nix+: :tangle nixosModules/hyprland-mode-switch-notifier/default.nix
:END:
...
******** TODO Check out [[https://github.com/rharish101/ReGreet][ReGreet]] :to_think:
...
******** Hyprland modes
:PROPERTIES:
:header-args:nix+: :tangle modules/hyprland/modes/default.nix
:END:

Imported [[(hyprland-modes-import)][here]].
...
********* Sound_ctl
:PROPERTIES:
:header-args:nix+: :tangle modules/hyprland/modes/sound_ctl.nix
:END:
...
********* Resize_ctl
:PROPERTIES:
:header-args:nix+: :tangle modules/hyprland/modes/resize_ctl.nix
:END:
...
mkModeBinding
  ...
  ''
#+end_src
********* TODO Workspace_edit :feat:
:PROPERTIES:
:header-args:nix+: :tangle modules/hyprland/modes/workspace_edit_ctl.nix
:END:

Mode to rename/renumber workspaces. For now I want to try working without a bar.
...
********* Power_ctl
:PROPERTIES:
:header-args:nix+: :tangle modules/hyprland/modes/power_ctl.nix
:END:
...
mkModeBinding
  ...
  ''
#+end_src
********** TODO Make hyprland exit clean -- stop all needed services :fix:
******** Hyprland lib
:PROPERTIES:
...
******** Hyprland mode switch notifier
:PROPERTIES:
:header-args:nix+: :tangle modules/hyprland/hyprland-mode-switch-notifier/default.nix
:END:

Imported [[(use-mode-switch-notifier-import)][here]].
...
******** Hyprland per-host configs

This section hosts machine-specific Hyprland configuration that is dynamically loaded [[(per-machine-hyprland-config)][here]].

********* Neptunium hyprland config
:PROPERTIES:
:header-args:nix+: :tangle modules/hyprland/per-host-configs/neptunium.nix
:END:
...
********* Uranium hyprland config
:PROPERTIES:
:header-args:nix+: :tangle modules/hyprland/per-host-configs/uranium.nix
:END:
...
******** Lockscreen
:PROPERTIES:
:header-args:nix+: :tangle modules/hyprland/lock/default.nix
:END:

Section configuring the lockscreen behavior.

Imported [[(lock-hyprland-import)][here]].
...
******** Hyprland theme
:PROPERTIES:
:header-args:nix+: :tangle modules/hyprland/theme/default.nix
:END:

Imported [[(hyprland-theme-import)][here]].
...
******** DONE port over sound control mode :feat:
CLOSED: [2023-06-06 Tue 10:48]

******** DONE [#A] Port over the resize mode :feat:
CLOSED: [2023-06-06 Tue 10:48]

******** TODO [#C] Consider a new launcher :feat:

******** DONE [#A] port over workspace edit/renumber/rename mode :feat:
CLOSED: [2023-06-06 Tue 11:34]
...
******** TODO [#C] Check out [[https://github.com/Slackadays/Clipboard][Clipboard]] :to_think:

******** DONE [#A] Make sure lock on suspend works
CLOSED: [2023-06-17 Sat 10:37]

******** DONE [[https://github.com/hyprwm/Hyprland/issues/1250][Ultrawide config]] :feat:
CLOSED: [2023-06-02 Fri 11:36]

******** TODO [#C] "which key" for modes :feat:

Whenever a mode is triggered -- it would be helpful to show a list of keybinds similar to emacs.

******** DONE GTK theme
CLOSED: [2023-07-10 Mon 11:35]
...
** Nix flake system management notes

*** ~nixos-rebuild switch --option substitute false --flake .~ :: to rebuild the system offline, without reaching out to nix cache

*** ~nixos-rebuild switch --rollback~ :: to roll back to previous gen

* Projects

** CI/CD

*** STRT Pre-commit checks :chore:
...
** DONE [#C] Consider merging tmux here :feat:
CLOSED: [2023-07-04 Tue 19:13]
...
** DONE Hyprland migration plan [9/9]
...
*** DONE Finish not low-priority tasks from [[*User hyprland config][User hyprland config]]
CLOSED: [2023-07-10 Mon 11:35]

*** DONE Migrate uranium to hyprland
CLOSED: [2023-06-16 Fri 11:43]
...
*** TODO [[https://github.com/fufexan/dotfiles/blob/main/home/wayland/hyprland/config.nix][Fufexan/dotfiles]]
